...
class DoctrineSectionReader extends Doctrine implements ReadSectionInterface
{
    ...
    {
        if (!empty($fields[$fieldProperty]) &&
            !empty($fields[$fieldProperty]['relationship']) &&
            !empty($fields[$fieldProperty]['relationship']['kind'])
        ) {
    ...
    private function manualQuery(OptionsInterface $readOptions): \ArrayIterator
    {
        ...
        $parameters = $readOptions->getQueryParameters();
